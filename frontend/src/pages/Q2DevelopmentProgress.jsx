...
export default function Q2DevelopmentProgress({ product = 'uta' }) {
  ...
  const sprintDef = SPRINT_DEFINITIONS.q2;
  ...
  useEffect(() => {
    const fetchEpics = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/q2-progress/${product}`);

        if (!response.ok) {
          throw new Error(`Failed to fetch Q2 progress: ${response.statusText}`);
        }
        ...
        console.error('Error fetching Q2 progress:', err);
        setError(err.message || 'Failed to load Q2 development progress');
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              {epics.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-gray-600">No epics found for Q2 development.</p>
                </div>
              ) : (
